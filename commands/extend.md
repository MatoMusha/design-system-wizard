...
**Preconditions (hard gate, checked before the interview):** populated `ds-manifest.json` (inventory + two-tier tokens + parity map); conventions in place; healthy token pipeline (defer to `/infra` if broken); clean-enough tree (code lands on a branch); `figc status` green. If any fails, stop and name the command that resolves it — never half-add a component.
...
1. **Build in Figma.** `@design-system-wizard:figc-operator` builds the component + variant matrix, **binds semantic tokens** to every color property (no hex, never `resize()` an instance), then `figc shot`-verifies the variants.
2. **Generate code.** `@design-system-wizard:component-generator` adds the confirmed Radix primitive / shadcn source, Tailwind classes referencing the **semantic** CSS-variable layer, matching the Figma variant matrix one-to-one.
...
4. **Canvas doc frame.** `@design-system-wizard:figma-doc-builder` renders the component's canvas doc frame (placed instances across variants, anatomy callouts, Do/Don't pairs).
...
7. **Verify parity.** `@design-system-wizard:parity-verifier` asserts name identity across every hop; a failure blocks completion.
8. **Final checkpoint.** Confirm with evidence: parity passes, the `figc shot` looks correct, the Storybook story renders. Update agent-readability artifacts so the component is discoverable, then report complete.
