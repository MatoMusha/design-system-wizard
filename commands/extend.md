...
8. **Craft QA.** `@design-system-wizard:craft-reviewer` `figc shot`s the component + its doc card and checks the **Craft Checklist** (no overlaps, on-grid spacing, radii/type from scale, Lucide icons, auto-layout, contrast AA); loops fixes through `figc-operator` until it passes.
