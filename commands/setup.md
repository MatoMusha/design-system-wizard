...
5. `@design-system-wizard:craft-reviewer` — **craft-verification gate.** `figc shot` the foundation pages and every component frame, inspect against the **Craft Checklist** (no overlaps; all spacing on-grid/token-bound; radii from scale; type from scale; Lucide icons on-grid; auto-layout everywhere; contrast AA), and loop fixes back through `figc-operator` until it passes. Do not report success until it does.
