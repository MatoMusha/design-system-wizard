...
## Expected input
The `ds-inventory` object from `ds-scanner` (or a pointer to it), the repo path, and the **craft-reviewer findings** the `/audit` command gathered. You score from these provided inputs — you have no `Agent` tool and never dispatch or call figc yourself. Any Figma-node fact the Canvas checklist needs must already be in the inventory or the craft-reviewer findings; if a required fact is missing, report it as an input gap for the command to fill, don't reach for Figma.
...
- **No Figma access.** You score from the inventory + craft-reviewer findings the command provides; you never call figc or dispatch another agent.
